...
const Dashboard = ({ user }) => {
  ...
  const [selectedVehicle, setSelectedVehicle] = useState(null); // Nuovo stato per veicolo selezionato
  const [showVehicleManager, setShowVehicleManager] = useState(false); // Controlla visualizzazione
  ...
  const checkUserVehicles = async () => {
    // Qui potremmo controllare se l'utente ha veicoli
    // Per ora mostriamo sempre il vehicle manager al primo accesso
  ...
  const handleVehicleSelect = (vehicle) => {
    ...
    // Quando un veicolo è selezionato, possiamo tornare alla dashboard normale
  ...
  return (
    <div className="dashboard">
      ...
      {selectedVehicle && (
        <div className="active-vehicle-section">
          <div className="section-header">
            <h2>🚗 Veicolo Attivo</h2>
            <button 
              className="manage-vehicles-btn"
          ...
          <div className="vehicle-quick-info">
            <div className="vehicle-detail">
              <strong>{selectedVehicle.nomeveicolo}</strong>
              <span>{selectedVehicle.targa || 'Nessuna targa'}</span>
            </div>
            <div className="vehicle-detail">
              <strong>Km Attuali</strong>
              <span>{selectedVehicle.kmattuali?.toLocaleString() || 0} km</span>
            </div>
            <div className="vehicle-detail">
              <strong>Ultimo Aggiornamento</strong>
              <span>
                {new Date(selectedVehicle.kmattualidataorainserimento).toLocaleDateString('it-IT')}
              </span>
            </div>
          </div>
        </div>
      ...
      {/* Azioni Rapide - AGGIUNTA Gestione Veicoli */}
        ...
        <div className="reminders-list">
          ...
          {selectedVehicle && (
            <div className="reminder-item">
              <div className="reminder-icon">🚗</div>
              <div className="reminder-content">
                <p><strong>Veicolo attivo</strong> - {selectedVehicle.nomeveicolo}</p>
                <span className="reminder-time">{selectedVehicle.kmattuali?.toLocaleString() || 0} km</span>
              </div>
            </div>
          )}
        </div>
...
export default Dashboard;          { id: 3, client: 'Anna Bianchi', service: 'Tagliando', price: 180, status: 'programmato', date: '2024-01-16' },
